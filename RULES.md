...
| | |
|---|---|
| **Rule** | No `tests/test_*.py` may reach the `fabric-target` wheel, directly or through any chain of this repo's own modules. Code that needs testing without a stack goes in a dependency-free module, like `platform/apipath.py` or `platform/say.py`; code that needs a stack imports it inside the function that talks to one. |
